...
{
	float xDelta = (float)newWidth / (float)width;
	float yDelta = (float)newHeight / (float)height;
...
void SimWindow::settings(lv_obj_t *obj, lv_event_t event)
{
	if (event != LV_EVENT_CLICKED)
		return;
	SimWindow *sw = (SimWindow*)lv_obj_get_user_data(obj);
...
void SimWindow::closeSettings(lv_obj_t *obj, lv_event_t event)
{
	if (event != LV_EVENT_CLICKED)
		return;
	lv_obj_t* win = (lv_obj_t*)lv_obj_get_user_data(obj);
