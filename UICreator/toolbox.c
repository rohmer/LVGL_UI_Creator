...
static lv_obj_t* last_widget = nullptr; //Use by set position.There may got a error, if we delete this.
...
void toolbox_win_init(lv_obj_t* parent)
{
    ...
    toolbox_win = lv_win_create(parent, nullptr);
    ...
    lv_obj_align(toolbox_win, nullptr, LV_ALIGN_IN_LEFT_MID, 0, 0);

    lv_obj_t* win_btn = lv_win_add_btn(toolbox_win, LV_SYMBOL_TRASH);
    lv_obj_set_event_cb(win_btn, create_undo);

    lv_obj_t* list = lv_list_create(toolbox_win, nullptr);
    ...
    lv_obj_t* th_cont = lv_cont_create(toolbox_win, nullptr);
    ...
    lv_obj_t* th_roller = lv_roller_create(th_cont, nullptr);
    lv_roller_set_options(th_roller, th_options, true);
    lv_obj_set_event_cb(th_roller, theme_select_event_handler);

    lv_obj_t* hue_roller = lv_roller_create(th_cont, nullptr);
...
static void create_label_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        ...
        lv_obj_t* newObj = lv_label_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        last_widget = newObj;
        lv_obj_set_drag(newObj, true);
        lv_obj_set_protect(newObj, LV_PROTECT_PRESS_LOST);
        lv_obj_set_event_cb(newObj, update_setting);

        widget_set_info(newObj, WIDGET_TYPE_LABEL);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_btn_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_btn_create(par, nullptr);
        // lv_label_set_text(lv_label_create(newObj, NULL), "Button");
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        // lv_obj_set_user_data(newObj, info);
        widget_set_info(newObj, WIDGET_TYPE_BTN);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_cb_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_cb_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        widget_set_info(newObj, WIDGET_TYPE_CB);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_ddlist_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_ddlist_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        lv_obj_t* scrl = lv_page_get_scrl(newObj);
        lv_obj_set_drag_parent(scrl, true);

        widget_set_info(newObj, WIDGET_TYPE_DDLIST);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_bar_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_bar_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }

        lv_bar_set_anim_time(newObj, 10000);
        lv_bar_set_value(newObj, 100, LV_ANIM_ON);
        last_widget = newObj;
        lv_obj_set_drag(newObj, true);
        lv_obj_set_protect(newObj, LV_PROTECT_PRESS_LOST);
        lv_obj_set_event_cb(newObj, update_setting);

        widget_set_info(newObj, WIDGET_TYPE_BAR);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_led_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_led_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        widget_set_info(newObj, WIDGET_TYPE_LED);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void create_gauge_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_gauge_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        widget_set_info(newObj, WIDGET_TYPE_GAUGE);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
//     if(ev == LV_EVENT_CLICKED)
//     {
//         lv_obj_t * newObj = lv_line_create(tft_win, NULL);
//         if(last_widget != NULL)
//         {
//             lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
//             lv_point_t p[2] = {{LV_DPI, LV_DPI}, {LV_DPI + 10, LV_DPI + 10}};
//             lv_line_set_points(newObj, p, 2);
//         }
//      last_widget = newObj;     
//         lv_obj_set_drag(newObj, true);
//         lv_obj_set_protect(newObj, LV_PROTECT_PRESS_LOST);
...
static void create_slider_cb(lv_obj_t* obj, lv_event_t ev)
{
    ...
    if (ev == LV_EVENT_CLICKED)
    {
        lv_obj_t* par = layerview_get_sel_obj();
        if (par == nullptr) return;
        lv_obj_t* newObj = lv_slider_create(par, nullptr);
        if (last_widget != nullptr)
        {
            lv_obj_set_pos(newObj, lv_obj_get_x(last_widget) + 10, lv_obj_get_y(last_widget) + 10);
        }
        ...
        widget_set_info(newObj, WIDGET_TYPE_SLIDER);
        layerview_add(layerview_get_sel_layer(), newObj);
    }
}
...
static void init_all_themes(uint16_t hue)
{
    ...
    int i = 0;
#if LV_USE_THEME_NIGHT
    themes[i++] = lv_theme_night_init(hue, nullptr);
#endif

#if LV_USE_THEME_MATERIAL
    themes[i++] = lv_theme_material_init(hue, nullptr);
#endif

#if LV_USE_THEME_ALIEN
    themes[i++] = lv_theme_alien_init(hue, nullptr);
#endif

#if LV_USE_THEME_ZEN
    themes[i++] = lv_theme_zen_init(hue, nullptr);
#endif

#if LV_USE_THEME_NEMO
    themes[i++] = lv_theme_nemo_init(hue, nullptr);
#endif

#if LV_USE_THEME_MONO
    themes[i++] = lv_theme_mono_init(hue, nullptr);
#endif

#if LV_USE_THEME_DEFAULT
    themes[i++] = lv_theme_default_init(hue, nullptr);
#endif
}
