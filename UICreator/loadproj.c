...
static widget_stack_t* wstack = nullptr;

static void sax_cb(mxml_node_t* node, mxml_sax_event_t event, void* data);
static void wstack_push(lv_obj_t* newObj);
...
void load_project(lv_obj_t* tft_win)
{
    ...
    wstack_push(tft_win);
    mxml_node_t* sax_node = mxmlSAXLoadFile(nullptr, fp, MXML_OPAQUE_CALLBACK, static_cast<mxml_sax_cb_t>(sax_cb),
                                            nullptr);
    fclose(fp);
}
...
static void sax_cb(mxml_node_t* node, mxml_sax_event_t event, void* data)
{
    if (event == MXML_SAX_ELEMENT_OPEN)
    {
        ...
        {
            lv_obj_t* obj = lv_btn_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "LABEL"))
        {
            lv_obj_t* obj = lv_label_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "LED"))
        {
            lv_obj_t* obj = lv_led_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "CHECKBOX"))
        {
            lv_obj_t* obj = lv_cb_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "BAR"))
        {
            lv_obj_t* obj = lv_bar_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "ROLLER"))
        {
            lv_obj_t* obj = lv_roller_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "DDLIST"))
        {
            lv_obj_t* obj = lv_ddlist_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "ARC"))
        {
            lv_obj_t* obj = lv_arc_create(wstack_top(), nullptr);
            wstack_push(obj);
        }
        if (!strcmp(element, "CONTAINER"))
        {
            lv_obj_t* obj = lv_cont_create(wstack_top(), nullptr);
...
static void wstack_push(lv_obj_t* newObj)
{
    widget_stack_t* new_top = static_cast<widget_stack_t *>(malloc(sizeof(widget_stack_t)));
    new_top->next = wstack;
    new_top->widget = newObj;
    wstack = new_top;
}
...
static lv_obj_t* wstack_top(void)
{
    if (wstack != nullptr) return wstack->widget;
    return nullptr;
}
