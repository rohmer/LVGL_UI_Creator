...
lv_obj_t* tbox_create(lv_obj_t* par, std::string title)
{
    lv_obj_t* cont = lv_cont_create(par, NULL);
    lv_cont_set_fit2(cont, LV_FIT_FLOOD, LV_FIT_TIGHT);
    lv_cont_set_layout(cont, LV_LAYOUT_ROW_M);

    tbox_ext_t* ext = (tbox_ext_t *)lv_obj_allocate_ext_attr(cont, sizeof(tbox_ext_t));
    ext->title = lv_label_create(cont, NULL);
    lv_label_set_text(ext->title, title.c_str());
...
{
    tbox_ext_t* ext = (tbox_ext_t *)lv_obj_get_ext_attr(tbox);
    return ext->ta;
}
...
lv_obj_t* layerview_add(lv_obj_t* par_layer, lv_obj_t* obj)
{
    ...
    layerview_ext_t* ext = (layerview_ext_t *)lv_obj_allocate_ext_attr(layer, sizeof(layerview_ext_t));
    ...
    layerview_ext_t* par_ext = (layerview_ext_t *)lv_obj_get_ext_attr(par_layer);
...
lv_obj_t* layerview_get_sel_obj(void)
{
    if (sel_layer)
    {
        layerview_ext_t* ext = (layerview_ext_t *)lv_obj_get_ext_attr(sel_layer);
        return ext->bind;
    }
    return NULL;
}
...
{
    layerview_ext_t* left_ext = (layerview_ext_t *)lv_obj_get_ext_attr(left_layer);
    if (left_ext->right == NULL)
    {
        layerview_ext_t* ext = (layerview_ext_t *)lv_obj_get_ext_attr(layer);
        ext->left = left_layer;
...
void layerview_del(lv_obj_t* layer) //Delete a layer and its child && move its brother layer
{
    ...
    if (layer == par_ext->child)
    {
        ...
        if (ext->right != NULL)
        {
            layerview_ext_t* right_ext = (layerview_ext_t *)lv_obj_get_ext_attr(ext->right);
            right_ext->left = NULL;
        }
    }
    else
    {
        layerview_ext_t* left_ext = (layerview_ext_t *)lv_obj_get_ext_attr(ext->left);
